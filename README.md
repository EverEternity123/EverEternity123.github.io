...
│   └── order.json        ★ 自定义的文章顺序（点「批量编辑」保存后才有这个文件）
...
4. 顶部会实时显示「待保存：顺序有调整 · 3 篇的标签改了」这样的摘要，
   改完点 **「保存全部改动」** —— 这是一次 commit（顺序和标签/隐藏分别落在
   `order.json` 和 `posts.json`，所以是两次 API 调用、同一条提交信息），
   约 1 分钟后线上生效。
5. 中途想放弃点「取消」，会从**进入那一刻的快照整体回滚** ——
...
而不是按文章。改这里的名字会**一次性改掉所有文章**上的这个标签（含未公开的）：

- 改成另一个已有标签的名字 = **合并**
- 留空 = **从所有文章上移除**

点「应用到列表」后立刻反映到文章列表里，但**还没有提交** ——
回到列表点「保存全部改动」才算数。
