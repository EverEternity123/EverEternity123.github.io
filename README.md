...
**文章**：被排过的（写在 `order.json` 的 `ids` 里）按你排的顺序；
没被排过的（＝新发的文章）**按日期插进对应的位置**。

```
被排过的     → 按你排的顺序
没被排过的   → 按日期插到上面那一串里它该在的地方（不是一律顶到最前面）
```

> 2026-09-21 改的：以前没排过的是一律排到最前面，于是补一篇 2019 年的旧文，
> 它会赖在首页最顶上。现在按日期落位，`order.json` 本身就是日期倒序时，
> 整表结果和「纯按日期排」完全一致 —— 所以**平时根本不用去管它**。

**标签**：被排过的按 `tags` 的顺序排在前面，没排过的按默认规则排在后面。
...
- **文章页写全**：`1234 字 · 约 4 分钟读完`

字数和时长同源 —— 都出自 `markdown.js` 的 `charCount()`（去掉代码块和空白后的
正文长度），所以「2 千字」和「约 6 分钟」永远不会互相打架。
